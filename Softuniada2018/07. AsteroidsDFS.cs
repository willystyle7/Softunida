...
namespace Asteroids
{
    class Asteroids
    {
            ...
            while (input != "end")
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    for (int j = 0; j < columns; j++)
                    {
                        ...
                        {
                            int asteroid = 0;
                            Traverse(matrix, rows, columns, i, j, ref asteroid);
        ...
        private static void Traverse(int[,] matrix, int rows, int columns, int row, int col, ref int asteroid)
        {
            asteroid++;
            matrix[row, col] = 0;
            ...
            //    }
            //}
            if ((row - 1 >= 0) && matrix[row - 1, col] != 0) Traverse(matrix, rows, columns, row - 1, col, ref asteroid);
            if ((row + 1 < rows) && matrix[row + 1, col] != 0) Traverse(matrix, rows, columns, row + 1, col, ref asteroid);
            if ((col - 1 >= 0) && matrix[row, col - 1] != 0) Traverse(matrix, rows, columns, row, col - 1, ref asteroid);
            if ((col + 1 < columns) && matrix[row, col + 1] != 0) Traverse(matrix, rows, columns, row, col + 1, ref asteroid);
            return;
        }
